...
class AdminDocumentsController extends Controller
{
    public function index()
    {
        ...
        foreach ($specialApprovedDocs as $doc) {
            ...
            // Parameter count (special docs don't have category breakdowns, they're all under the parameter)
            if ($doc->parameter_id) {
                if (!isset($parameterApprovedCounts[$doc->parameter_id])) $parameterApprovedCounts[$doc->parameter_id] = 0;
                $parameterApprovedCounts[$doc->parameter_id]++;
            }
        ...
        return Inertia::render('admin/documents_approved', [
            'sidebar' => $sidebar,
    ...
    public function approvedDocuments(Request $request)
    {
        ...
        // Check if admin is assigned to this program/area
    ...
    public function pendingPage(Request $request)
    {
        ...
        return Inertia::render('admin/documents_pending', [
            'sidebar' => $sidebar,
            'csrfToken' => csrf_token(),
        ]);
    }
    ...
    public function pendingDocuments(Request $request)
    {
        ...
        if ($request->has('program_id') && $request->has('area_id')) {
            ...
            if ($isSpecialParameter) {
                // Fetch from special_documents table
                $query = SpecialDocument::where('status', 'pending')
                    ->where('program_id', $program_id)
                    ->where('area_id', $area_id)
                ...
                $documents = $query->orderBy('created_at', 'desc')
                    ...
                    ->map(function ($doc) use ($user, $parameter) {
                        ...
                        return [
                            'id' => $doc->id,
                            'filename' => $doc->doc_filename,
                            'url' => $doc->doc_filename ? Storage::url("{$folderPath}/{$doc->doc_filename}") : null,
                            'video_filename' => null,
                            'video_url' => null,
                            'uploaded_at' => $doc->created_at->toDateTimeString(),
                            'user_name' => $doc->user->name ?? '',
    ...
    public function disapprovedPage(Request $request)
    {
        ...
        return Inertia::render('admin/documents_disapproved', [
            'sidebar' => $sidebar,
            'csrfToken' => csrf_token(),
        ]);
    }
            ...
            $documents = $query->orderBy('created_at', 'desc')
                ...
                ->map(function ($doc) use ($parameter) {
                    ...
                    return [
                        ...
                        'disapproved_by' => optional($doc->checkedBy)->name,
    ...
    public function dashboardData(Request $request)
    {
        ...
        // Admin should see system-wide statistics, not just assigned programs
        $totalDocuments = Document::count();
        
        $pendingReview = Document::where('status', 'pending')->count();
        
        $approvedDocuments = Document::where('status', 'approved')->count();
        
        $disapprovedDocuments = Document::where('status', 'disapproved')->count();
        ...
        // Count total programs in the system
        $totalPrograms = \App\Models\Program::count();

        // System activities count (placeholder)
        $systemActivities = $totalDocuments; // Can be customized based on what you consider "activities"

        // Get recent activities (last 10 system-wide activities)
        $recentDocuments = Document::with(['program:id,name,code', 'area:id,name,code', 'parameter:id,name,code', 'user:id,name'])
        ...
        foreach ($recentDocuments as $doc) {
            ...
            if ($doc->status === 'pending') {
                ...
                $activity['title'] = 'New document uploaded';
                $activity['description'] = 'Document uploaded by ' . ($doc->user->name ?? 'Unknown User') . ' in ' . ($doc->program->name ?? 'Unknown Program');
            } elseif ($doc->status === 'approved') {
                $activity['type'] = 'document_approved';
                $activity['title'] = 'Document approved';
                $activity['description'] = 'Document approved in ' . ($doc->program->name ?? 'Unknown Program');
            } elseif ($doc->status === 'disapproved') {
                $activity['type'] = 'document_disapproved';
                $activity['title'] = 'Document disapproved';
                $activity['description'] = 'Document disapproved in ' . ($doc->program->name ?? 'Unknown Program');
            } else {
