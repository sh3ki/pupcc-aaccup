...
use App\Models\Landing\ExhibitFacultyManual;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;
use Inertia\Inertia;

class ExhibitFacultyManualController extends Controller
{
    /**
     * Display the admin exhibit faculty manual management page
     */
    public function index()
    {
        $content = ExhibitFacultyManual::getContent();
        ...
        return Inertia::render('admin/layout/ExhibitFacultyManual', [
            'facultyManualContent' => (object) $transformedContent,
        ]);
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->hasFile('hero_image')) {
            ...
            $oldContent = ExhibitFacultyManual::getContent();
            if ($oldContent->hero_image && Storage::exists($oldContent->hero_image)) {
                Storage::delete($oldContent->hero_image);
            }
            
            $data['hero_image'] = $request->file('hero_image')->store('exhibit/faculty-manual/hero', 'public');
        }
        ...
        if ($request->hasFile('manual_document')) {
            ...
            $data['manual_document'] = $request->file('manual_document')->store('exhibit/faculty-manual/documents', 'public');
        }
        ...
        if ($request->hasFile('footer_image')) {
            ...
            $data['footer_image'] = $request->file('footer_image')->store('exhibit/faculty-manual/footer', 'public');
        }

        ExhibitFacultyManual::updateContent($data);

        return back()->with('message', "Faculty Manual content updated successfully!");
    }
    ...
    public function show()
    {
        ...
        return Inertia::render('landing/exhibit/facultymanual', [
            'facultyManualContent' => (object) $transformedContent,
        ]);
    }
    ...
    {
        $manual = ExhibitFacultyManual::first();
        $content = ExhibitFacultyManual::getContent();
