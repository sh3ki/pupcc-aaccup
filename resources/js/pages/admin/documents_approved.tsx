...
export default function AdminDocuments(props: PageProps) {
    ...
    const handleViewPendingDoc = (docId: number) => {
        ...
        fetch(`/admin/documents/pending/${docId}`, {
            headers: { 'Accept': 'application/json' },
            credentials: 'same-origin',
        })
            .then(res => res.json())
    ...
    useEffect(() => {
        ...
        if (selected.programId && selected.areaId && selected.parameterId && selected.category) {
            setLoadingDocs(true);
            fetch(`/admin/documents/approved?program_id=${selected.programId}&area_id=${selected.areaId}&parameter_id=${selected.parameterId}&category=${selected.category}`, {
                headers: { 'Accept': 'application/json' },
                credentials: 'same-origin',
            })
                .then(res => res.json())
            ...
            setLoadingDocs(true);
            fetch(`/admin/documents/approved?program_id=${selected.programId}&area_id=${selected.areaId}`, {
                headers: { 'Accept': 'application/json' },
                credentials: 'same-origin',
            })
                .then(res => res.json())
                ...
                <div className="flex w-full min-h-[calc(100vh-64px-40px)] overflow-hidden">
                    ...
                    <section className="flex-1 w-full px-6 py-4 text-left flex flex-col min-h-[calc(100vh-64px-40px)] max-h-[calc(100vh-64px-40px)] overflow-auto">
                        <div className="flex items-center justify-between flex-shrink-0">
                            <div>
                                {!selected.programId ? (
                                    <h1 className="text-xl font-bold text-[#7F0404]">Admin Documents</h1>
                                ) : selectedProgram ? (
