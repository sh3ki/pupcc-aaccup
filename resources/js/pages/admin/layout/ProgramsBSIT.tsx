...
export default function ProgramsBSIT({ bsitContent }: Props) {
    ...
    // Hero Section State
    const [heroTitle, setHeroTitle] = useState(bsitContent.hero_title || 'Bachelor of Science in Information Technology (BSIT)');
    const [heroSubtitle, setHeroSubtitle] = useState(bsitContent.hero_subtitle || 'Empowering future IT professionals with cutting-edge technology and innovation.');
    const [heroImage, setHeroImage] = useState<string | File>(bsitContent.hero_image || '');

    // Overview Section State
    const [overviewSectionTitle, setOverviewSectionTitle] = useState(bsitContent.overview_section_title || 'Program Overview');
    const [programDescription, setProgramDescription] = useState(bsitContent.program_description || '');
    const [programImage, setProgramImage] = useState<string | File>(bsitContent.program_image || '');

    // Objectives Section State
    const [objectivesSectionTitle, setObjectivesSectionTitle] = useState(bsitContent.objectives_section_title || 'Program Objectives');
    const [objectives, setObjectives] = useState<string[]>(
        bsitContent.objectives_data && bsitContent.objectives_data.length > 0 
            ? bsitContent.objectives_data 
            : ['']
    );

    // AVP Section State
    const [avpSectionTitle, setAvpSectionTitle] = useState(bsitContent.avp_section_title || 'Program AVP');
    const [programVideo, setProgramVideo] = useState(bsitContent.program_video || '');
    const [programVideoType, setProgramVideoType] = useState<'youtube' | 'upload'>(bsitContent.program_video_type || 'youtube');
    const [programVideoFile, setProgramVideoFile] = useState<File | null>(null);

    // Action Section State
    const [actionSectionTitle, setActionSectionTitle] = useState(bsitContent.action_section_title || 'Program in Action');
    const [actionImages, setActionImages] = useState<(string | File)[]>(
        bsitContent.action_images && bsitContent.action_images.length > 0 
            ? bsitContent.action_images 
            : ['', '', '', '']
    );

    // Graduates Section State
    const [graduatesSectionTitle, setGraduatesSectionTitle] = useState(bsitContent.graduates_section_title || 'Notable Graduates');
    const [graduates, setGraduates] = useState<GraduateItem[]>(
        bsitContent.graduates_data && bsitContent.graduates_data.length > 0 
            ? bsitContent.graduates_data 
            : [{ name: '', video: '', video_type: 'youtube' }]
    );

    // Accreditation Section State
    const [accreditationSectionTitle, setAccreditationSectionTitle] = useState(bsitContent.accreditation_section_title || 'Accreditation Areas');
    const [accreditationAreas, setAccreditationAreas] = useState<AccreditationArea[]>(
        bsitContent.accreditation_areas && bsitContent.accreditation_areas.length > 0 
            ? bsitContent.accreditation_areas 
            : [{ title: '', image: '' }]
    );
    ...
    const handleSave = () => {
        ...
        router.post('/admin/layout/programs/bsit', formData, {
            forceFormData: true,
                ...
                <div className="flex w-full h-[calc(100vh-64px-40px)]">
                    ...
                    <aside className="w-1/4 bg-white border-r border-gray-200 h-full sticky top-16 self-start overflow-y-auto">
                        <div className="p-6">
                            {/* Header */}
                            <div className="mb-6">
                                <h2 className="text-xl font-bold text-gray-800">BSIT Program Editor</h2>
                    ...
                    <section className="flex-1 w-full px-8 py-4 pb-2 text-left flex flex-col h-full">
                        ...
                        <div className="flex justify-between items-center mb-4 flex-shrink-0">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-800">BSIT Program Preview</h1>
                        ...
                        <div className="flex-1 min-h-0 flex justify-center">
                            <div className="h-full w-full max-w-7xl">
                                <PagePreview 
                                    pageUrl="/programs/bsit" 
                                    title="BSIT Program Preview"
                                    key={previewKey}
                                />
                            </div>
                        </div>
