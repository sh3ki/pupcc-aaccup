...
export default function LayoutExhibitFacultyManual({ facultyManualContent }: Props) {
    ...
    // Hero Section State
    const [heroTitle, setHeroTitle] = useState(facultyManualContent.hero_title || "Faculty Manual");
    const [heroSubtitle, setHeroSubtitle] = useState(facultyManualContent.hero_subtitle || 'Comprehensive guidelines and procedures for faculty members');
    const [heroImage, setHeroImage] = useState<string | File>(facultyManualContent.hero_image || '');

    // Manual Section State
    const [sectionTitle, setSectionTitle] = useState(facultyManualContent.section_title || "Faculty Manual Preview");
    const [manualDocument, setManualDocument] = useState<string | File>(facultyManualContent.manual_document || '');

    // Footer Section State
    const [footerSectionTitle, setFooterSectionTitle] = useState(facultyManualContent.footer_section_title || 'Mula Sayo, Para Sa Bayan');
    const [footerImage, setFooterImage] = useState<string | File>(facultyManualContent.footer_image || '');
    ...
    const handleSave = () => {
        ...
        router.post('/admin/layout/exhibit/faculty-manual', formData, {
            forceFormData: true,
                ...
                <div className="flex w-full h-[calc(100vh-64px-40px)]">
                    ...
                    <aside className="w-1/4 bg-white border-r border-gray-200 h-full sticky top-16 self-start overflow-y-auto">
                        <div className="p-6">
                            {/* Header */}
                            <div className="mb-6">
                                <h2 className="text-xl font-bold text-gray-800">Faculty Manual Page Editor</h2>
                    ...
                    <section className="flex-1 w-full px-8 py-4 pb-2 text-left flex flex-col h-full">
                        ...
                        <div className="flex justify-between items-center mb-4 flex-shrink-0">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-800">Faculty Manual Page Preview</h1>
                        ...
                        <div className="flex-1 min-h-0 flex justify-center">
                            <div className="h-full w-full max-w-7xl">
                                <PagePreview 
                                    pageUrl="/exhibit/faculty-manual" 
                                    title="Faculty Manual Page Preview"
                                    key={previewKey}
                                />
                            </div>
                        </div>
