...
interface InstructionalMaterialsContent {
    ...
    materials_document: string | File;
...
export default function LayoutExhibitInstructionalMaterials({ instructionalMaterialsContent }: Props) {
    ...
    // Hero Section State
    const [heroTitle, setHeroTitle] = useState(instructionalMaterialsContent.hero_title || "Instructional Materials");
    const [heroSubtitle, setHeroSubtitle] = useState(instructionalMaterialsContent.hero_subtitle || 'Educational resources and materials for effective learning');
    const [heroImage, setHeroImage] = useState<string | File>(instructionalMaterialsContent.hero_image || '');

    // Materials Section State
    const [sectionTitle, setSectionTitle] = useState(instructionalMaterialsContent.section_title || "Instructional Materials Preview");
    const [materialsDocument, setMaterialsDocument] = useState<string | File>(instructionalMaterialsContent.materials_document || '');
    ...
    const handleSave = () => {
        ...
        // Add materials document
        if (materialsDocument instanceof File) {
            formData.append('materials_document', materialsDocument);
        }
        ...
        router.post('/admin/layout/exhibit/instructional-materials', formData, {
            forceFormData: true,
    ...
    const sections = [
        ...
        { id: 'materials', name: 'Materials Document' },
                ...
                <div className="flex w-full h-[calc(100vh-64px-40px)]">
                    ...
                    <aside className="w-1/4 bg-white border-r border-gray-200 h-full sticky top-16 self-start overflow-y-auto">
                        <div className="p-6">
                            {/* Header */}
                            <div className="mb-6">
                                <h2 className="text-xl font-bold text-gray-800">Instructional Materials Page Editor</h2>
                            ...
                            <div className="space-y-6">
                                ...
                                {/* Materials Document Section */}
                                {activeSection === 'materials' && (
                                    <div>
                                        <h3 className="text-lg font-semibold text-[#7F0404] mb-4">Materials Document</h3>
                                        <div className="space-y-4">
                                            <div>
                                            ...
                                            <FileUpload
                                                label="Materials Document"
                                                value={materialsDocument}
                                                onChange={(file) => setMaterialsDocument(file || '')}
                                            ...
                                            <p className="text-xs text-gray-500 mt-1">
                                                Upload materials document (Image, PDF, DOC, or DOCX)
                                            </p>
                                        </div>
                                    </div>
                                )}
                    ...
                    <section className="flex-1 w-full px-8 py-4 pb-2 text-left flex flex-col h-full">
                        ...
                        <div className="flex justify-between items-center mb-4 flex-shrink-0">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-800">Instructional Materials Page Preview</h1>
                        ...
                        <div className="flex-1 min-h-0 flex justify-center">
                            <div className="h-full w-full max-w-7xl">
                                <PagePreview 
                                    pageUrl="/exhibit/instructional-materials" 
                                    title="Instructional Materials Page Preview"
                                    key={previewKey}
                                />
                            </div>
                        </div>
