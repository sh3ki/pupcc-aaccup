...
export default function LayoutExhibitAdministrativeManual({ administrativeManualContent }: Props) {
    ...
    // Hero Section State
    const [heroTitle, setHeroTitle] = useState(administrativeManualContent.hero_title || "Administrative Manual");
    const [heroSubtitle, setHeroSubtitle] = useState(administrativeManualContent.hero_subtitle || 'Comprehensive guidelines and procedures for administrative operations');
    const [heroImage, setHeroImage] = useState<string | File>(administrativeManualContent.hero_image || '');

    // Manual Section State
    const [sectionTitle, setSectionTitle] = useState(administrativeManualContent.section_title || "Administrative Manual Preview");
    const [manualDocument, setManualDocument] = useState<string | File>(administrativeManualContent.manual_document || '');

    // Footer Section State
    const [footerSectionTitle, setFooterSectionTitle] = useState(administrativeManualContent.footer_section_title || 'Mula Sayo, Para Sa Bayan');
    const [footerImage, setFooterImage] = useState<string | File>(administrativeManualContent.footer_image || '');
    ...
    const handleSave = () => {
        ...
        // Add manual document
        if (manualDocument instanceof File) {
            formData.append('manual_document', manualDocument);
        }
        ...
        router.post('/admin/layout/exhibit/administrative-manual', formData, {
            forceFormData: true,
    ...
    const sections = [
        ...
        { id: 'manual', name: 'Manual Document' },
                ...
                <div className="flex w-full h-[calc(100vh-64px-40px)]">
                    ...
                    <aside className="w-1/4 bg-white border-r border-gray-200 h-full sticky top-16 self-start overflow-y-auto">
                        <div className="p-6">
                            {/* Header */}
                            <div className="mb-6">
                                <h2 className="text-xl font-bold text-gray-800">Administrative Manual Page Editor</h2>
                            ...
                            <div className="space-y-6">
                                ...
                                {activeSection === 'manual' && (
                                    <div>
                                        <h3 className="text-lg font-semibold text-[#7F0404] mb-4">Manual Document</h3>
                                        <div className="space-y-4">
                                            <div>
                                            ...
                                            <FileUpload
                                                ...
                                                onChange={(file) => setManualDocument(file || '')}
                                            ...
                                            <p className="text-xs text-gray-500 mt-1">
                                                Upload manual document (Image, PDF, DOC, or DOCX)
                                            </p>
                                        </div>
                                    </div>
                                )}
                    ...
                    <section className="flex-1 w-full px-8 py-4 pb-2 text-left flex flex-col h-full">
                        ...
                        <div className="flex justify-between items-center mb-4 flex-shrink-0">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-800">Administrative Manual Page Preview</h1>
                        ...
                        <div className="flex-1 min-h-0 flex justify-center">
                            <div className="h-full w-full max-w-7xl">
                                <PagePreview 
                                    pageUrl="/exhibit/administrative-manual" 
                                    title="Administrative Manual Page Preview"
                                    key={previewKey}
                                />
                            </div>
                        </div>
