...
export default function LayoutExhibitCitizensCharter({ charterContent }: Props) {
    ...
    // Hero Section State
    const [heroTitle, setHeroTitle] = useState(charterContent.hero_title || "Citizen's Charter");
    const [heroSubtitle, setHeroSubtitle] = useState(charterContent.hero_subtitle || 'Official guidelines and procedures for citizens accessing PUP Calauan services');
    const [heroImage, setHeroImage] = useState<string | File>(charterContent.hero_image || '');

    // Charter Section State
    const [sectionTitle, setSectionTitle] = useState(charterContent.section_title || "Citizen's Charter Preview");
    const [charterDocument, setCharterDocument] = useState<string | File>(charterContent.charter_document || '');

    // Footer Section State
    const [footerSectionTitle, setFooterSectionTitle] = useState(charterContent.footer_section_title || 'Mula Sayo, Para Sa Bayan');
    const [footerImage, setFooterImage] = useState<string | File>(charterContent.footer_image || '');
    ...
    const handleSave = () => {
        ...
        if (charterDocument instanceof File) {
            formData.append('charter_document', charterDocument);
        }
        ...
        router.post('/admin/layout/exhibit/citizens-charter', formData, {
            forceFormData: true,
    ...
    const sections = [
        ...
        { id: 'charter', name: 'Charter Document' },
                ...
                <div className="flex w-full h-[calc(100vh-64px-40px)]">
                    ...
                    <aside className="w-1/4 bg-white border-r border-gray-200 h-full sticky top-16 self-start overflow-y-auto">
                        <div className="p-6">
                            {/* Header */}
                            <div className="mb-6">
                                <h2 className="text-xl font-bold text-gray-800">Citizen Charter Page Editor</h2>
                            ...
                            <div className="space-y-6">
                                ...
                                {activeSection === 'charter' && (
                                    <div>
                                        <h3 className="text-lg font-semibold text-[#7F0404] mb-4">Charter Document</h3>
                                        <div className="space-y-4">
                                            <div>
                                            ...
                                            <FileUpload
                                                ...
                                                value={charterDocument}
                                                onChange={(file) => setCharterDocument(file || '')}
                                            ...
                                            <p className="text-xs text-gray-500 mt-1">
                                                Upload charter document (Image, PDF, DOC, or DOCX)
                                            </p>
                                        </div>
                                    </div>
                                )}
                    ...
                    <section className="flex-1 w-full px-8 py-4 pb-2 text-left flex flex-col h-full">
                        ...
                        <div className="flex justify-between items-center mb-4 flex-shrink-0">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-800">Citizen Charter Page Preview</h1>
                        ...
                        <div className="flex-1 min-h-0 flex justify-center">
                            <div className="h-full w-full max-w-7xl">
                                <PagePreview 
                                    pageUrl="/exhibit/citizens-charter" 
                                    title="Citizen Charter Page Preview"
                                    key={previewKey}
                                />
                            </div>
                        </div>
