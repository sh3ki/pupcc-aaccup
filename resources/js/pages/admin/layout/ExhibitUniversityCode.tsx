...
export default function LayoutExhibitUniversityCode({ universityCodeContent }: Props) {
    ...
    // Hero Section State
    const [heroTitle, setHeroTitle] = useState(universityCodeContent.hero_title || "University Code");
    const [heroSubtitle, setHeroSubtitle] = useState(universityCodeContent.hero_subtitle || 'Official university code of conduct and regulations');
    const [heroImage, setHeroImage] = useState<string | File>(universityCodeContent.hero_image || '');

    // Code Section State
    const [sectionTitle, setSectionTitle] = useState(universityCodeContent.section_title || "University Code Preview");
    const [codeDocument, setCodeDocument] = useState<string | File>(universityCodeContent.code_document || '');

    // Footer Section State
    const [footerSectionTitle, setFooterSectionTitle] = useState(universityCodeContent.footer_section_title || 'Mula Sayo, Para Sa Bayan');
    const [footerImage, setFooterImage] = useState<string | File>(universityCodeContent.footer_image || '');
    ...
    const handleSave = () => {
        ...
        // Add code document
        if (codeDocument instanceof File) {
            formData.append('code_document', codeDocument);
        }
        ...
        router.post('/admin/layout/exhibit/university-code', formData, {
            forceFormData: true,
    ...
    const sections = [
        ...
        { id: 'code', name: 'Code Document' },
        { id: 'footer', name: 'Mula Sayo' }
    ];

    return (
        <>
            <Head title="Layout: Exhibit - University Code" />
            <DashboardLayout>
                <div className="flex w-full h-[calc(100vh-64px-40px)]">
                    ...
                    <aside className="w-1/4 bg-white border-r border-gray-200 h-full sticky top-16 self-start overflow-y-auto">
                        <div className="p-6">
                            {/* Header */}
                            <div className="mb-6">
                                <h2 className="text-xl font-bold text-gray-800">University Code Page Editor</h2>
                            ...
                            <div className="space-y-6">
                                ...
                                {/* Code Document Section */}
                                {activeSection === 'code' && (
                                    <div>
                                        <h3 className="text-lg font-semibold text-[#7F0404] mb-4">Code Document</h3>
                                        <div className="space-y-4">
                                            <div>
                                            ...
                                            <FileUpload
                                                label="Code Document"
                                                value={codeDocument}
                                                onChange={(file) => setCodeDocument(file || '')}
                                            ...
                                            <p className="text-xs text-gray-500 mt-1">
                                                Upload code document (Image, PDF, DOC, or DOCX)
                                            </p>
                                        </div>
                                    </div>
                                )}
                    ...
                    <section className="flex-1 w-full px-8 py-4 pb-2 text-left flex flex-col h-full">
                        ...
                        <div className="flex justify-between items-center mb-4 flex-shrink-0">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-800">University Code Page Preview</h1>
                        ...
                        <div className="flex-1 min-h-0 flex justify-center">
                            <div className="h-full w-full max-w-7xl">
                                <PagePreview 
                                    pageUrl="/exhibit/university-code" 
                                    title="University Code Page Preview"
                                    key={previewKey}
                                />
                            </div>
                        </div>
