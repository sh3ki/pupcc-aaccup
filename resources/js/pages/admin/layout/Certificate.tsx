...
interface CertificateContent {
    ...
    certificate_document: string | File;
...
export default function LayoutCertificate({ certificateContent }: Props) {
    ...
    // Hero Section State
    const [heroTitle, setHeroTitle] = useState(certificateContent.hero_title || 'Certificate of Authenticity');
    const [heroSubtitle, setHeroSubtitle] = useState(certificateContent.hero_subtitle || 'Official proof of authenticity for PUP Calauan documentation');
    const [heroImage, setHeroImage] = useState<string | File>(certificateContent.hero_image || '');

    // Certificate Section State
    const [sectionTitle, setSectionTitle] = useState(certificateContent.section_title || 'Certificate Preview');
    const [certificateDocument, setCertificateDocument] = useState<string | File>(certificateContent.certificate_document || '');

    // Footer Section State
    const [footerSectionTitle, setFooterSectionTitle] = useState(certificateContent.footer_section_title || 'Mula Sayo, Para Sa Bayan');
    const [footerImage, setFooterImage] = useState<string | File>(certificateContent.footer_image || '');
    ...
    const handleSave = () => {
        ...
        router.post('/admin/layout/certificate', formData, {
            forceFormData: true,
    ...
    const sections = [
        ...
        { id: 'certificate', name: 'Certificate Document' },
                ...
                <div className="flex w-full h-[calc(100vh-64px-40px)]">
                    ...
                    <aside className="w-1/4 bg-white border-r border-gray-200 h-full sticky top-16 self-start overflow-y-auto">
                        <div className="p-6">
                            {/* Header */}
                            <div className="mb-6">
                                <h2 className="text-xl font-bold text-gray-800">Certificate Page Editor</h2>
                            ...
                            <div className="space-y-6">
                                ...
                                {activeSection === 'certificate' && (
                                    <div>
                                        <h3 className="text-lg font-semibold text-[#7F0404] mb-4">Certificate Document</h3>
                                        <div className="space-y-4">
                                            <div>
                                            ...
                                            <FileUpload
                                                ...
                                                onChange={(file) => setCertificateDocument(file || '')}
                                                ...
                                                maxSize={10}
                                                description="Upload certificate document (Image, PDF, DOC, or DOCX)"
                                            />
                                        </div>
                                    </div>
                                )}
                    ...
                    <section className="flex-1 w-full px-8 py-4 pb-2 text-left flex flex-col h-full">
                        ...
                        <div className="flex justify-between items-center mb-4 flex-shrink-0">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-800">Certificate Page Preview</h1>
                        ...
                        <div className="flex-1 min-h-0 flex justify-center">
                            <div className="h-full w-full max-w-7xl">
                                <PagePreview 
                                    pageUrl="/certificate" 
                                    title="Certificate Page Preview"
                                    key={previewKey}
                                />
                            </div>
                        </div>
