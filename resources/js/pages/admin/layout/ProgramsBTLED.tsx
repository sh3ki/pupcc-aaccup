...
export default function ProgramsBTLED({ btledContent }: Props) {
    ...
    // Hero Section State
    const [heroTitle, setHeroTitle] = useState(btledContent.hero_title || 'Bachelor of Technology and Livelihood Education (BTLED)');
    const [heroSubtitle, setHeroSubtitle] = useState(btledContent.hero_subtitle || 'Developing competent teachers in technology and livelihood education for the modern classroom.');
    const [heroImage, setHeroImage] = useState<string | File>(btledContent.hero_image || '');

    // Overview Section State
    const [overviewSectionTitle, setOverviewSectionTitle] = useState(btledContent.overview_section_title || 'Program Overview');
    const [programDescription, setProgramDescription] = useState(btledContent.program_description || '');
    const [programImage, setProgramImage] = useState<string | File>(btledContent.program_image || '');

    // Objectives Section State
    const [objectivesSectionTitle, setObjectivesSectionTitle] = useState(btledContent.objectives_section_title || 'Program Objectives');
    const [objectives, setObjectives] = useState<string[]>(
        btledContent.objectives_data && btledContent.objectives_data.length > 0 
            ? btledContent.objectives_data 
            : ['']
    );

    // AVP Section State
    const [avpSectionTitle, setAvpSectionTitle] = useState(btledContent.avp_section_title || 'Program AVP');
    const [programVideo, setProgramVideo] = useState(btledContent.program_video || '');
    const [programVideoType, setProgramVideoType] = useState<'youtube' | 'upload'>(btledContent.program_video_type || 'youtube');
    const [programVideoFile, setProgramVideoFile] = useState<File | null>(null);

    // Action Section State
    const [actionSectionTitle, setActionSectionTitle] = useState(btledContent.action_section_title || 'Program in Action');
    const [actionImages, setActionImages] = useState<(string | File)[]>(
        btledContent.action_images && btledContent.action_images.length > 0 
            ? btledContent.action_images 
            : ['', '', '', '']
    );

    // Graduates Section State
    const [graduatesSectionTitle, setGraduatesSectionTitle] = useState(btledContent.graduates_section_title || 'Notable Graduates');
    const [graduates, setGraduates] = useState<GraduateItem[]>(
        btledContent.graduates_data && btledContent.graduates_data.length > 0 
            ? btledContent.graduates_data 
            : [{ name: '', video: '', video_type: 'youtube' }]
    );

    // Accreditation Section State
    const [accreditationSectionTitle, setAccreditationSectionTitle] = useState(btledContent.accreditation_section_title || 'Accreditation Areas');
    const [accreditationAreas, setAccreditationAreas] = useState<AccreditationArea[]>(
        btledContent.accreditation_areas && btledContent.accreditation_areas.length > 0 
            ? btledContent.accreditation_areas 
            : [{ title: '', image: '' }]
    );

    // Mula Sayo Section State
    const [mulaSayoTitle, setMulaSayoTitle] = useState(btledContent.mula_sayo_title || 'Mula Sayo, Para Sa Bayan');
    const [mulaSayoImage, setMulaSayoImage] = useState<string | File>(btledContent.mula_sayo_image || '');
    ...
    const handleSave = () => {
        ...
        router.post('/admin/layout/programs/btled', formData, {
            forceFormData: true,
                ...
                <div className="flex w-full h-[calc(100vh-64px-40px)]">
                    ...
                    <aside className="w-1/4 bg-white border-r border-gray-200 h-full sticky top-16 self-start overflow-y-auto">
                        <div className="p-6">
                            {/* Header */}
                            <div className="mb-6">
                                <h2 className="text-xl font-bold text-gray-800">BTLED Program Editor</h2>
                    ...
                    <section className="flex-1 w-full px-8 py-4 pb-2 text-left flex flex-col h-full">
                        ...
                        <div className="flex justify-between items-center mb-4 flex-shrink-0">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-800">BTLED Program Preview</h1>
                        ...
                        <div className="flex-1 min-h-0 flex justify-center">
                            <div className="h-full w-full max-w-7xl">
                                <PagePreview 
                                    pageUrl="/programs/btled" 
                                    title="BTLED Program Preview"
                                    key={previewKey}
                                />
                            </div>
                        </div>
