...
export default function AboutAccreditation({ accreditationContent }: Props) {
    ...
    // Hero Section State
    const [heroTitle, setHeroTitle] = useState(accreditationContent.hero_title || 'Accreditation Task Force');
    const [heroSubtitle, setHeroSubtitle] = useState(accreditationContent.hero_subtitle || 'Leading excellence in quality assurance and institutional development');
    const [heroImage, setHeroImage] = useState<string | File>(accreditationContent.hero_image || '');

    // Faculty Section State
    const [facultySectionTitle, setFacultySectionTitle] = useState(accreditationContent.faculty_section_title || 'Faculty Members');
    const [faculty, setFaculty] = useState<FacultyItem[]>(
        accreditationContent.faculty_data && accreditationContent.faculty_data.length > 0 
            ? accreditationContent.faculty_data 
            : [{ image: '', name: '' }]
    );

    // Mula Sayo Section State
    const [mulaSayoTitle, setMulaSayoTitle] = useState(accreditationContent.mula_sayo_title || 'Mula Sayo, Para Sa Bayan');
    const [mulaSayoImage, setMulaSayoImage] = useState<string | File>(accreditationContent.mula_sayo_image || '');
    ...
    const handleSave = () => {
        ...
        router.post('/admin/layout/about/accreditation', formData, {
            forceFormData: true,
    ...
    return (
        <>
            <Head title="Layout: About - Accreditation" />
            <DashboardLayout>
                <div className="flex w-full h-[calc(100vh-64px-40px)]">
                    ...
                    <aside className="w-1/4 bg-white border-r border-gray-200 h-full sticky top-16 self-start overflow-y-auto">
                        <div className="p-6">
                            {/* Header */}
                            <div className="mb-6">
                                <h2 className="text-xl font-bold text-gray-800">Accreditation Page Editor</h2>
                    ...
                    <section className="flex-1 w-full px-8 py-4 pb-2 text-left flex flex-col h-full">
                        ...
                        <div className="flex justify-between items-center mb-4 flex-shrink-0">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-800">Accreditation Page Preview</h1>
                        ...
                        <div className="flex-1 min-h-0 flex justify-center">
                            <div className="h-full w-full max-w-7xl">
                                <PagePreview 
                                    pageUrl="/faculty/accreditation" 
                                    title="Accreditation Page Preview"
                                    key={previewKey}
                                />
                            </div>
                        </div>
