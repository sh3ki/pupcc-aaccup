...
export default function LayoutExhibitUniversityPolicies({ universityPoliciesContent }: Props) {
    ...
    // Hero Section State
    const [heroTitle, setHeroTitle] = useState(universityPoliciesContent.hero_title || "University Policies");
    const [heroSubtitle, setHeroSubtitle] = useState(universityPoliciesContent.hero_subtitle || 'Comprehensive policies and guidelines for university operations');
    const [heroImage, setHeroImage] = useState<string | File>(universityPoliciesContent.hero_image || '');

    // Policies Section State
    const [sectionTitle, setSectionTitle] = useState(universityPoliciesContent.section_title || "University Policies Preview");
    const [policiesDocument, setPoliciesDocument] = useState<string | File>(universityPoliciesContent.policies_document || '');

    // Footer Section State
    const [footerSectionTitle, setFooterSectionTitle] = useState(universityPoliciesContent.footer_section_title || 'Mula Sayo, Para Sa Bayan');
    const [footerImage, setFooterImage] = useState<string | File>(universityPoliciesContent.footer_image || '');
    ...
    const handleSave = () => {
        ...
        if (policiesDocument instanceof File) {
            formData.append('policies_document', policiesDocument);
        }
        ...
        router.post('/admin/layout/exhibit/university-policies', formData, {
            forceFormData: true,
    ...
    const sections = [
        ...
        { id: 'policies', name: 'Policies Document' },
        { id: 'footer', name: 'Mula Sayo' }
    ];

    return (
        <>
            <Head title="Layout: Exhibit - University Policies" />
            <DashboardLayout>
                <div className="flex w-full h-[calc(100vh-64px-40px)]">
                    ...
                    <aside className="w-1/4 bg-white border-r border-gray-200 h-full sticky top-16 self-start overflow-y-auto">
                        <div className="p-6">
                            {/* Header */}
                            <div className="mb-6">
                                <h2 className="text-xl font-bold text-gray-800">University Policies Page Editor</h2>
                            ...
                            <div className="space-y-6">
                                ...
                                {/* Policies Document Section */}
                                {activeSection === 'policies' && (
                                    <div>
                                        <h3 className="text-lg font-semibold text-[#7F0404] mb-4">Policies Document</h3>
                                        <div className="space-y-4">
                                            <div>
                                            ...
                                            <FileUpload
                                                ...
                                                value={policiesDocument}
                                                onChange={(file) => setPoliciesDocument(file || '')}
                                            ...
                                            <p className="text-xs text-gray-500 mt-1">
                                                Upload policies document (Image, PDF, DOC, or DOCX)
                                            </p>
                                        </div>
                                    </div>
                                )}
                    ...
                    <section className="flex-1 w-full px-8 py-4 pb-2 text-left flex flex-col h-full">
                        ...
                        <div className="flex justify-between items-center mb-4 flex-shrink-0">
                            <div>
                                <h1 className="text-2xl font-bold text-gray-800">University Policies Page Preview</h1>
                        ...
                        <div className="flex-1 min-h-0 flex justify-center">
                            <div className="h-full w-full max-w-7xl">
                                <PagePreview 
                                    pageUrl="/exhibit/university-policies" 
                                    title="University Policies Page Preview"
                                    key={previewKey}
                                />
                            </div>
                        </div>
