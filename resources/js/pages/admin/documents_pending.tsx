...
export default function AdminDocumentsPending(props: PageProps) {
    ...
    const fetchPendingTable = (page = 1) => {
        ...
        fetch(`/admin/documents/pending/data?page=${page}`, {
    ...
    const confirmDeletePending = async () => {
        ...
        try {
            ...
            const res = await fetch(`/admin/documents/pending/${docId}`, {
                method: 'DELETE',
                headers: {
    ...
    useEffect(() => {
        ...
        if (shouldFetch) {
            ...
            fetch(`/admin/documents/pending/data?${queryParams.toString()}`, {
                headers: { 'Accept': 'application/json' },
                credentials: 'same-origin',
            })
                .then(res => res.json())
            ...
            setLoadingDocs(true);
            fetch(`/admin/documents/pending/data?program_id=${selected.programId}&area_id=${selected.areaId}`, {
                headers: { 'Accept': 'application/json' },
                credentials: 'same-origin',
            })
                .then(res => res.json())
    ...
    useEffect(() => {
        ...
        function refreshPendingDocs() {
            ...
            fetch('/admin/documents/pending/sidebar', {
                headers: { 'Accept': 'application/json' },
                credentials: 'same-origin',
            })
                .then(res => res.json())
            ...
            if (selected.programId && selected.areaId && selected.parameterId && selected.category) {
                fetch(`/admin/documents/pending/data?program_id=${selected.programId}&area_id=${selected.areaId}&parameter_id=${selected.parameterId}&category=${selected.category}`, {
                    headers: { 'Accept': 'application/json' },
                    credentials: 'same-origin',
                })
                    .then(res => res.json())
    ...
    const confirmAction = async () => {
        ...
        setActionError('');
        try {
            const res = await fetch(`/admin/documents/pending/${currentDoc.id}/status`, {
                method: 'PATCH',
                headers: {
    ...
    return (
        <>
            <Head title="Admin Pending Documents" />
            <DashboardLayout>                
                <div className="flex w-full min-h-[calc(100vh-64px-40px)] overflow-hidden">
                    ...
                    <section className="flex-1 w-full px-6 py-4 text-left flex flex-col min-h-[calc(100vh-64px-40px)] max-h-[calc(100vh-64px-40px)] overflow-auto">
                        <div className="flex items-center justify-between flex-shrink-0">
                            <div>
                                {!selected.programId ? (
                                    <h1 className="text-xl font-bold text-[#7F0404]">Admin Pending Documents</h1>
                                ) : selectedProgram ? (
