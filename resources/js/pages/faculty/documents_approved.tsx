...
type Area = { id: number; name: string; code?: string; parameters?: Parameter[]; approved_count?: number };
type Program = { id: number; name: string; code?: string; areas: Area[]; approved_count?: number };
interface PageProps {
        sidebar: Program[];
        csrfToken: string;
    }
...
export default function FacultyDocuments(props: PageProps) {
        ...
        const [approvedDocs, setApprovedDocs] = useState<{ id: number, filename: string, url: string, uploaded_at: string, user_name?: string, approved_by?: string | null, approved_at?: string | null, updated_at?: string | null, parameter_id?: number, category?: string }[]>([]);
        const [viewerIndex, setViewerIndex] = useState(0);
        const [loadingDocs, setLoadingDocs] = useState(false);

    // No search field in this clone
    // page input not shown in this UI clone
        ...
        const filteredDocs = useMemo(() => {
            ...
            // no text search filtering on this page
            return docs;
    ...
    // navigation helper removed (not used)
    // navigation helpers (not rendered as buttons in this clone)
    ...
    // grid toggle handled via setGridOpen only when needed
    ...
    // no external video time handler needed
                    ...
                    <div className="flex w-full min-h-[calc(100vh-64px-40px)] overflow-hidden">
                        ...
                        <section className="flex-1 w-full px-6 py-4 text-left flex flex-col min-h-[calc(100vh-64px-40px)] max-h-[calc(100vh-64px-40px)] overflow-auto">
                            <div className="flex items-center justify-between flex-shrink-0">
                                <div>
                                    {!selected.programId ? (
                                        <h1 className="text-xl font-bold text-[#7F0404]">Faculty Documents</h1>
                                    ) : selectedProgram ? (
                                                    ...
                                                    <div className="mt-auto">
                                                        <div className="text-gray-600 mb-4">
                                                            ...
                                                            <div className="flex justify-between items-center">
                                                                ...
                                                                <span className={`font-semibold ${(program.approved_count ?? 0) > 0 ? 'text-green-600' : 'text-gray-500'}`}>
                                                                    {program.approved_count ?? 0}
                                                                </span>
                                                            </div>
                                                        </div>
