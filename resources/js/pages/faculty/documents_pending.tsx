...
export default function FacultyDocumentsPending(props: PageProps) {
    ...
    const fetchPendingTable = (page = 1) => {
        ...
        fetch(`/faculty/documents/pending/data?page=${page}`, {
    ...
    const confirmDeletePending = async () => {
        ...
        try {
            ...
            const res = await fetch(`/faculty/documents/pending/${docId}`, {
                method: 'DELETE',
                headers: {
    ...
    useEffect(() => {
        ...
        if (selected.programId && selected.areaId && selected.parameterId && 
            ...
            fetch(`/faculty/documents/pending/data?${params.join('&')}`, {
                headers: { 'Accept': 'application/json' },
                credentials: 'same-origin',
            })
                .then(res => res.json())
    ...
    useEffect(() => {
        ...
        function refreshPendingDocs() {
            ...
            fetch('/faculty/documents/pending/sidebar', {
                headers: { 'Accept': 'application/json' },
                credentials: 'same-origin',
            })
                .then(res => res.json())
    ...
    // Note: Approve/Disapprove functionality removed per requirement.

    return (
        <>
            <Head title="Faculty Pending Documents" />
            <DashboardLayout>                
                <div className="flex w-full min-h-[calc(100vh-64px-40px)] overflow-hidden">
                    ...
                    <section className="flex-1 w-full px-6 py-4 text-left flex flex-col min-h-[calc(100vh-64px-40px)] max-h-[calc(100vh-64px-40px)] overflow-auto">
                        <div className="flex items-center justify-between flex-shrink-0">
                            <div>
                                {!selected.programId ? (
                                    <h1 className="text-xl font-bold text-[#7F0404]">Faculty Pending Documents</h1>
                                ) : selectedProgram ? (
                            ...
                            <div className="flex gap-2">
                                ...
                                {/* Approve/Disapprove controls removed */}
                            </div>
                        </div>
