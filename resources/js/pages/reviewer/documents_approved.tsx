...
export default function ReviewerDocuments(props: PageProps) {
    ...
    useEffect(() => {
        if (pendingModalOpen) {
            ...
            setPendingError('');
            fetch('/dashboard/reviewer/documents/pending', {
    ...
    const handleViewPendingDoc = (docId: number) => {
        ...
        fetch(`/dashboard/reviewer/documents/pending/${docId}`, {
            headers: { 'Accept': 'application/json' },
            credentials: 'same-origin',
        })
            .then(res => res.json())
    ...
    useEffect(() => {
        if (selected.programId && selected.areaId) {
            setLoadingDocs(true);
            fetch(`/dashboard/reviewer/documents/approved?program_id=${selected.programId}&area_id=${selected.areaId}`, {
                headers: { 'Accept': 'application/json' },
                credentials: 'same-origin',
            })
                .then(res => res.json())
    ...
    const handleUploadSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await fetch('/dashboard/reviewer/documents/upload', {
                method: 'POST',
                body: formData,
                headers: {
                ...
                <div className="flex w-full min-h-[calc(100vh-64px-40px)]">
                    ...
                    <section className="flex-1 w-full px-8 py-4 text-left flex flex-col min-h-[calc(100vh-64px-40px)] max-h-[calc(100vh-64px-40px)]">
                        <div className="flex items-center justify-between flex-shrink-0">
                            <div>
                                {!selected.programId ? (
                                    <h1 className="text-4xl font-bold text-[#7F0404]">Reviewer Documents</h1>
                                ) : selectedProgram ? (
                        ...
                        {!selected.programId ? (
                            <p className="text-lg text-gray-700 mb-8">This is the documents page for reviewers.</p>
                        ) : null}
