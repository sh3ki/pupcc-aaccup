...
export default function Copc({ copcContent }: Props) {
    ...
    const copcUrl = copcContent.copc_document || "https://via.placeholder.com/900x1200/f3f4f6/6b7280?text=COPC+Preview";
    
    // Hero Section Data
    const heroImageUrl = copcContent.hero_image || "https://via.placeholder.com/1600x500/7f0404/ffffff?text=COPC+Hero+Image";
    
    // Footer Section Data  
    const footerImageUrl = copcContent.footer_image || "https://via.placeholder.com/1600x400/7f0404/ffffff?text=Footer+Background";
    ...
    useEffect(() => {
        if (copcContent.copc_document && isPDF(copcContent.copc_document) && !pdfLoaded) {
            setPdfLoaded(true);
        }
    }, [copcContent.copc_document, pdfLoaded]);

    // COPC Display Component - moved outside of animation effects
    const CopcDisplay = useCallback(() => {
        // Check if we have a real COPC (not placeholder)
        const hasRealCopc = copcContent.copc_document && 
                                  !copcContent.copc_document.includes('placeholder');
            ...
            return (
                <div className="flex flex-col items-center justify-center min-h-[400px] bg-gray-100 rounded-xl border border-gray-200">
                    <div className="text-center">
                        ...
                        <p className="text-gray-500 font-medium">No COPC uploaded</p>
                        <p className="text-gray-400 text-sm">Upload a COPC via the admin panel</p>
                    </div>
                </div>
            );
        }

        if (isPDF(copcContent.copc_document || '') && pdfLoaded) {
            return (
                <div className="w-full">
                    ...
                    <div className="mt-4 text-center">
                        <a
                            href={copcContent.copc_document}
                            target="_blank"
        ...
        } else if (isPDF(copcContent.copc_document || '') && !pdfLoaded) {
        ...
        if (isImage(copcContent.copc_document || '')) {
            return (
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] bg-gray-100 rounded-xl border border-gray-200">
                <div className="text-center">
                    ...
                    <p className="text-gray-500 text-sm mb-4">Click to download: {getFileExtension(copcUrl).toUpperCase()} file</p>
                    <a
                        href={copcUrl}
                        download
    ...
    }, [copcContent.copc_document, pdfLoaded, currentPage, setTotalPages, zoom, rotate, copcUrl]);
    
    // Animation hooks for different sections
    const [heroRef, heroVisible] = useScrollAnimation();
    const [copcRef, copcVisible] = useScrollAnimation();

    return (
        <>
            <Head title="Certificate of Program Compliance" />
