...
interface StudentHandbookContent {
    hero_image?: string;
    hero_title: string;
    hero_subtitle: string;
    section_title: string;
    handbook_document?: string;
    footer_section_title: string;
    footer_image?: string;
}

interface Props {
    studentHandbookContent: StudentHandbookContent;
}
...
export default function StudentHandbook({ studentHandbookContent }: Props) {
    // Use proper placeholder URLs that actually work
    const handbookUrl = studentHandbookContent.handbook_document || "https://via.placeholder.com/900x1200/f3f4f6/6b7280?text=Handbook+Preview";
    
    // Hero Section Data
    const heroImageUrl = studentHandbookContent.hero_image || "https://via.placeholder.com/1600x500/7f0404/ffffff?text=Student+Handbook+Hero+Image";
    
    // Footer Section Data  
    const footerImageUrl = studentHandbookContent.footer_image || "https://via.placeholder.com/1600x400/7f0404/ffffff?text=Footer+Background";
    ...
    useEffect(() => {
        if (studentHandbookContent.handbook_document && isPDF(studentHandbookContent.handbook_document) && !pdfLoaded) {
            setPdfLoaded(true);
        }
    }, [studentHandbookContent.handbook_document, pdfLoaded]);

    // Handbook Display Component - moved outside of animation effects
    const HandbookDisplay = useCallback(() => {
        // Check if we have a real handbook (not placeholder)
        const hasRealHandbook = studentHandbookContent.handbook_document && 
                                !studentHandbookContent.handbook_document.includes('placeholder');
            ...
            return (
                <div className="flex flex-col items-center justify-center min-h-[400px] bg-gray-100 rounded-xl border border-gray-200">
                    <div className="text-center">
                        ...
                        <p className="text-gray-500 font-medium">No handbook uploaded</p>
                        <p className="text-gray-400 text-sm">Upload a handbook via the admin panel</p>
                    </div>
                </div>
            );
        }

        if (isPDF(studentHandbookContent.handbook_document || '') && pdfLoaded) {
            return (
                <div className="w-full">
                    ...
                    <div className="w-full max-w-4xl mx-auto rounded-xl border border-gray-200 overflow-hidden" style={{ height: '800px' }}>
                        <PdfViewer
                            url={studentHandbookContent.handbook_document || ''}
                    ...
                    <div className="mt-4 text-center">
                        <a
                            href={studentHandbookContent.handbook_document}
                            target="_blank"
        ...
        } else if (isPDF(studentHandbookContent.handbook_document || '') && !pdfLoaded) {
        ...
        if (isImage(studentHandbookContent.handbook_document || '')) {
            return (
                <div className="w-full max-w-4xl mx-auto bg-gray-50 flex items-center justify-center rounded-xl border border-gray-200 overflow-hidden" style={{ minHeight: '400px' }}>
                    <img
                        src={studentHandbookContent.handbook_document}
                        alt="Handbook"
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] bg-gray-100 rounded-xl border border-gray-200">
                <div className="text-center">
                    ...
                    <p className="text-gray-500 text-sm mb-4">Click to download: {getFileExtension(handbookUrl).toUpperCase()} file</p>
                    <a
                        href={handbookUrl}
                        download
    ...
    }, [studentHandbookContent.handbook_document, pdfLoaded, currentPage, setTotalPages, zoom, rotate, handbookUrl]);
    
    // Animation hooks for different sections
    const [heroRef, heroVisible] = useScrollAnimation();
    const [handbookRef, handbookVisible] = useScrollAnimation();

    return (
        <>
            <Head title="Student Handbook" />
            <div className="min-h-screen bg-white overflow-x-hidden">
                    ...
                    >
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-10">
                            <h1 className={`text-5xl sm:text-6xl md:text-7xl font-bold text-white mb-4 drop-shadow-lg transition-all duration-1000 delay-300 ${
                                ...
                                {studentHandbookContent.hero_title}
                            </h1>
                            <p className={`text-xl sm:text-2xl md:text-3xl text-white/90 max-w-2xl mx-auto transition-all duration-1000 delay-500 ${
                                ...
                                {studentHandbookContent.hero_subtitle}
                            </p>
                        </div>
                    </section>
                            ...
                            <div className={`bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl border-t-4 transition-all duration-1000 delay-200 ${
                                ...
                                <div className="p-8 sm:p-12 flex flex-col items-center">
                                    <h2 className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-8 sm:mb-12 text-center transition-all duration-1000 delay-400 ${
                                        ...
                                        {studentHandbookContent.section_title}
                                    </h2>
                                    {/* Handbook Display with PDF Support */}
                ...
                <section className="relative py-16 sm:py-20 lg:py-24 px-0 transition-all duration-1200">
                    <div className="absolute inset-0 w-full h-full">
                        <img
                            src={footerImageUrl}
                            alt={studentHandbookContent.footer_section_title}
                    ...
                    <div className="relative z-10 flex flex-col items-center justify-center h-full">
                        <h2 className="text-3xl sm:text-5xl lg:text-6xl font-bold text-white text-shadow-lg mb-4 animate-fade-in-up">
                            {studentHandbookContent.footer_section_title}
                        </h2>
                    </div>
                </section>
