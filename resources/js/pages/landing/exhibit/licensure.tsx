...
export default function Licensure({ licensureContent }: Props) {
    // Use proper placeholder URLs that actually work
    const licensureUrl = licensureContent.licensure_document || "https://via.placeholder.com/900x1200/f3f4f6/6b7280?text=Licensure+Preview";
    
    // Hero Section Data
    const heroImageUrl = licensureContent.hero_image || "https://via.placeholder.com/1600x500/7f0404/ffffff?text=Licensure+Hero+Image";
    
    // Footer Section Data  
    const footerImageUrl = licensureContent.footer_image || "https://via.placeholder.com/1600x400/7f0404/ffffff?text=Footer+Background";
    ...
    useEffect(() => {
        if (licensureContent.licensure_document && isPDF(licensureContent.licensure_document) && !pdfLoaded) {
            setPdfLoaded(true);
        }
    }, [licensureContent.licensure_document, pdfLoaded]);

    // Licensure Display Component - moved outside of animation effects
    const LicensureDisplay = useCallback(() => {
        // Check if we have a real licensure (not placeholder)
        const hasRealLicensure = licensureContent.licensure_document && 
                                 !licensureContent.licensure_document.includes('placeholder');
            ...
            return (
                <div className="flex flex-col items-center justify-center min-h-[400px] bg-gray-100 rounded-xl border border-gray-200">
                    <div className="text-center">
                        ...
                        <p className="text-gray-500 font-medium">No licensure uploaded</p>
                        <p className="text-gray-400 text-sm">Upload a licensure document via the admin panel</p>
                    </div>
                </div>
            );
        }

        if (isPDF(licensureContent.licensure_document || '') && pdfLoaded) {
            return (
                <div className="w-full">
                    ...
                    <div className="w-full max-w-4xl mx-auto rounded-xl border border-gray-200 overflow-hidden" style={{ height: '800px' }}>
                        <PdfViewer
                            url={licensureContent.licensure_document || ''}
                    ...
                    <div className="mt-4 text-center">
                        <a
                            href={licensureContent.licensure_document}
                            target="_blank"
        ...
        } else if (isPDF(licensureContent.licensure_document || '') && !pdfLoaded) {
        ...
        if (isImage(licensureContent.licensure_document || '')) {
            return (
                <div className="w-full max-w-4xl mx-auto bg-gray-50 flex items-center justify-center rounded-xl border border-gray-200 overflow-hidden" style={{ minHeight: '400px' }}>
                    <img
                        src={licensureContent.licensure_document}
                        alt="Licensure"
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] bg-gray-100 rounded-xl border border-gray-200">
                <div className="text-center">
                    ...
                    <p className="text-gray-500 text-sm mb-4">Click to download: {getFileExtension(licensureUrl).toUpperCase()} file</p>
                    <a
                        href={licensureUrl}
                        download
    ...
    }, [licensureContent.licensure_document, pdfLoaded, currentPage, setTotalPages, zoom, rotate, licensureUrl]);
    
    // Animation hooks for different sections
    const [heroRef, heroVisible] = useScrollAnimation();
    const [licensureRef, licensureVisible] = useScrollAnimation();

    return (
        <>
            <Head title="Licensure" />
            <div className="min-h-screen bg-white overflow-x-hidden">
                    ...
                    >
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center z-10">
                            ...
                            <p className={`text-xl sm:text-2xl md:text-3xl text-white/90 max-w-2xl mx-auto transition-all duration-1000 delay-500 ${
                                ...
                                {licensureContent.hero_subtitle}
                            </p>
                        </div>
                    </section>
                            ...
                            <div className={`bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl border-t-4 transition-all duration-1000 delay-200 ${
                                ...
                                <div className="p-8 sm:p-12 flex flex-col items-center">
                                    <h2 className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-8 sm:mb-12 text-center transition-all duration-1000 delay-400 ${
                                        ...
                                        {licensureContent.section_title}
                                    </h2>
                                    {/* Licensure Display with PDF Support */}
                ...
                <section className="relative py-16 sm:py-20 lg:py-24 px-0 transition-all duration-1200">
                    <div className="absolute inset-0 w-full h-full">
                        <img
                            src={footerImageUrl}
                            alt={licensureContent.footer_section_title}
                    ...
                    <div className="relative z-10 flex flex-col items-center justify-center h-full">
                        <h2 className="text-3xl sm:text-5xl lg:text-6xl font-bold text-white text-shadow-lg mb-4 animate-fade-in-up">
                            {licensureContent.footer_section_title}
                        </h2>
                    </div>
                </section>
