...
export default function BTLEDFaculty({ btledContent }: Props) {
    ...
    useEffect(() => {
        if (btledContent) {
            preloadLandingResources(btledContent);
        }
    }, [btledContent]);
    ...
    return (
        <>
            <Head title="BTLED Faculty">
                <meta name="description" content="Meet our dedicated BTLED faculty members" />
                
                {/* Preload critical resources */}
                <link rel="preload" href={btledContent.hero_image || "/api/placeholder/1600/800"} as="image" />
                {btledContent.faculty_data?.slice(0, 4).map((faculty, index) => (
                    <link
                ...
                <link rel="preload" href={btledContent.mula_sayo_image} as="image" />
